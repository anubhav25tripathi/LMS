...
const StudentsEnrolled = () => {
  ...
  const fetchstudentsEnrolled= async ()=>{
    try{
      ...
      if(data.success){
        setStudentsEnrolled(data.enrolledStudents.reverse());
      }
      else{
  ...
  return studentsEnrolled ? (
    <div className="min-h-screen flex flex-col items-start justify-between md:p-8  p-4 pt-8 pb-0">
     <div className="flex flex-col items-center max-w-4xl w-full overflow-hidden rounded-md bg-white border border-gray-500/20 ">
      <table className="text-gray-900 border-b border-gray-500/20 text-sm text-left w-full md:table-auto table-fixed">
        ...
        <tbody className="text-sm text-gray-500">
          {studentsEnrolled.map((item,index)=>(
             <tr key={index} className="border-b border-gray-500/20">
              ...
              <td className="md:px-4 px-2 py-3 flex items-center space-x-3"><img src={item.student.imageUrl} className='w-9 h-9 rounded-full' alt="student image" /><span className='truncate'>{item.student.name}</span></td>
