                ...
                <div key={chapterIndex} className="bg-white border rounded-1g mb-4" >
                    <div className="flex justify-between items-center p-4 border-b">
                        <div className='flex items-center' >
                           ...
                           className={`mr-2 cursor-pointer transition-all ${chapter.collapsed &&"-rotate-90"}`} onClick={()=>handleChapter('toggle',chapter.chapterId)} />
                           <span className="font-semibold">{chapterIndex + 1} {chapter.chapterTitle}</span>
                        </div>
                       <span className='text-gray-500'>{chapter.chapterContent.length} Lectures</span>
                       <img src={assets.cross_icon} alt="" className='cursor-pointer w-5 h-5' onClick={()=>handleChapter('remove',chapter.chapterId)} />
                    </div>
                    {!chapter.collapsed &&
                      <div className="p-4">
                ...
                showPopup && (
                  <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
                    <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                      ...
                      <div className="mb-2">
                         <p>Is Preview Free </p>
                         <input type="checkbox" className="mt-1 block w-full border rounded py-1 px-2" value={lectureDetails.isPreviewFree}
                      ...
                      <img onClick={() => setShowPopup(false)} src={assets.cross_icon}
                       className='absolute top-4 right-4 w-4 cursor-pointer h-5' alt="'" />
                    </div>
