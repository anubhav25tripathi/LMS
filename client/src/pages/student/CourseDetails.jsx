...
const CourseDetails = () => {
  ...
  const {allCourses, calculaterating,calculateChapterTime,calculateCourseDuration,
         calculateNoOfLecture,backendUrl,userData,getToken,}= useContext(AppContext)
      ...
      <div className='max-w-xl z-10 text-gray-500'>
        ...
        <div className='flex items-center space-x-2'>
                ...
                <div className='flex '>
                  ...
                  {[...Array(5)].map((_, index) =>  (<img src={index<Math.floor(calculaterating(courseData))? assets.star : assets.emptystar} alt="Star" key={index} className='w-3.5 h-3.5' />) )}
                </div>
                <p>{courseData.courseRatings.length}{courseData.courseRatings.length>1 ? ' ratings' : ' rating'}</p>
                <p>{courseData.enrolledStudents.length}{courseData.enrolledStudents.length>1 ? ' students': ' student'}</p>
        </div>
        <p className='text-sm'>CourseBy <span className='text-blue-600 underline'>{courseData.educator.name}</span></p>
       ...
       <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
        ...
        <div className='p-5'>
          <div className='flex items-center gap-2'>
            <img src={assets.time_left} alt="time-left-clock-icon" className='w-3 h-3' />
            <p className='text-red-500'><span className='font-medium'>5 Days</span> left at this price !</p>
          </div>
          <div className='flex items-center gap-3 pt-2'>
            <p className='text-gray-800 md:text-4xl text-2xl font-semibold'>$ {(courseData.coursePrice-courseData.coursePrice*courseData.discount/100).toFixed(2)}</p>
            <p className='md:text-lg text-gray-500 line-through'>$ {courseData.coursePrice}</p>
            <p className='md:text-lg text-gray-500'>{courseData.discount}% off</p>
          </div>
          <div className='flex items-center text-sm md:text-default gap-7 pt-2 md:pt-4 text-gray-500'>
            ...
            <div className='flex items-center gap-1'>
              <img src={assets.clock} alt="star" className='w-3 h-3' />
              <p>{calculateCourseDuration(courseData)}</p>
            </div>
            <div className='h-4 w-px bg-gray-500/40'></div>
            
            <div className='flex items-center gap-1'>
              <img src={assets.book} alt="star" className='w-4 h-3' />
              <p>{calculateNoOfLecture(courseData)} lessons</p>
            </div>
            
          </div>
          <button onClick={enrollCourse} className='md:mt-6 mt-4 w-full py-3 rounded bg-blue-600 text-white font-medium'>{isAlreadyEnrolled ? 'AlreadyEnrolled':'Enroll'}</button>
          <div className='pt-6'>
             <p className='md:text-xl text-lg font-medium text-gray-800'>What's in the course ?</p>
             <ul className='ml-4 pt-2 text-sm md:text-default text-gray-500 list-disc'>
              <li>Lifetime access with free updates</li>
              <li>step-by-step, hands-on project guidance</li>
              <li>Downlodable resouses, and sourse code.</li>
...
// ✅ Utility function to correctly extract YouTube video ID
                        ...
                        <div className='flex justify-between items-center w-full text-gray-800 text-xs md:text-default'>
                          ...
                          <div className='flex gap-2'>
                            {lecture.isPreviewFree && (
                              <p 
                                onClick={()=>
                                  setPlayerData({videoId: extractYouTubeVideoId(lecture.lectureUrl)})
                                }
